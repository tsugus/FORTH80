 ...
 6 P (           ( Requires dedicated "virtual machine" )
 7 P (                                                             )
...
10 LIST
 ...
 3 P ( the max bytes of tokens )   32 2 + CONSTANT MAXTOKEN
 4 P ( word buffer size [> C/L] )  64 6 + CONSTANT WORDBUFSIZE
 5 P ( PAD size )                  80 1 + CONSTANT PADSIZE
 6 P ( temporary buffer area size )
 7 P                WORDBUFSIZE PADSIZE + CONSTANT TMPBUFSIZE
 8 P  
 9 P ( the Inner Words for Inner Interpreter )
10 P  ( WPUSH ( --- W AX ; W @  AX @  NEXT )
11 P  ( APUSH ( --- AX ; AX @  NEXT )
12 P  ( NEXT  ( --- ; IP @ @ BX !  2 IP +!  NEXT1 )
13 P  ( NEXT1 ( --- ; BX @ W !  1 W +!  BX @ @ PC ! )
14 P  
15 P -->
...
12 LIST
 ...
 3 P    7 0010 ! ( +2  revision No. )
 4 P 0000 0012 ! ( +4  user version [0..255]*256+[0..25])
 5 P    ( 0014 ) ( +6  S0 )
 6 P    ( 0016 ) ( +8  R0 )
 7 P    ( 0018 ) ( +10 TIB )
...
29 LIST
 ...
 4 P           200 CONSTANT B/BUF  ( bytes per buffer )
 5 P             2 CONSTANT B/SCR  ( blocks per screen )
...
34 LIST
 0 P ( ?TERMINAL READ-REC WRITE-REC )
 1 P : ?TERMINAL ( --- f ) CTST [ CTST @ HERE 2- ! ] ;
 2 P : READ-REC ( n1 a n2 --- f ; ) READ [ READ @ HERE 2- ! ] ;
 3 P : WRITE-REC ( n1 a n2 --- f ; ) WRITE [ WRITE @ HERE 2- ! ] ;
 4 P  
 5 P ( set the end of word search at START's lfa )
 6 P 0 ' START LFA !
 7 P  
 8 P  
 9 P  
...
15 P   IF DROP HERE LATEST (FIND) THEN ;  ." 50 " -->
...
15 P LATEST PFA CFA 3 th_patch @ !  ." 51 " -->
...
15 P WORD-UPDATE FORGET IMMEDIATE  ." 67 " -->
...
15 P WORD-UPDATE FORGET REPEAT   ." 71 " -->
...
77 LIST
 ...
 5 P (               |   .   |                                     )
 6 P (               |=======|                                     )
 7 P (       LIT-6 ->| 005CH |     start of dictionary             )
 8 P (               |   .   |                                     )
 9 P (               |   .   |                                     )
...
10 P (               |   .   |                                     )
11 P (               | 7B16H |     bottom of stack                 )
12 P (               |=======|                                     )
13 P ( INITS0, TIB ->| 7B18H |     terminal input buffer           )
14 P (               |   .   |                                     )
15 P (               |   .   |                                     )
...
79 LIST
 ...
 2 P (               |   .   |                                     )
 3 P (               | 7BB6H |     bottom of return stack          )
 4 P (               |=======|                                     )
 5 P (  INITR0, UP ->| 7BB8H |     top of user variables area      )
 6 P (               |   :   |                                     )
 7 P (               |=======|                                     )
 8 P (       FIRST ->| 7BF8H |     top of disk buffers             )
 9 P (               |   :   |                                     )
