...
- [FORTH80　 Ver. 0.8.1](#forth80-ver-081)
    - [Abstract](#abstract)
  - [これは何？](#これは何)
  - [使い方](#使い方)
  - [セルフホスティング](#セルフホスティング)
    - [補足（FORTH 初心者の方は読み飛ばしてください）](#補足forth-初心者の方は読み飛ばしてください)
  - [起動オプション](#起動オプション)
    - [-h](#-h)
    - [-s](#-s)
    - [-u](#-u)
...
FORTH80.fth がそのための中心的な FORTH プログラムコードです。
...
しかし逆に言えば、いくら自動とはいえ、最初から DRIVE1.img に書き込んでおけばこのような手間は必要ないのですが、そこはソースコードの管理上、そうしたくない理由がこちらにあるのです。
再びご了承ください。
...
7. 「./VSM -u < start.txt&crarr;」で FORTH80 を起動してセルフホスティングを実行。自動で FORTH80.bin を上書き。
...
### 補足（FORTH 初心者の方は読み飛ばしてください）

このセルフホスティングには、若干、問題点があります。
スタックがディクショナリを上書きしてしまう可能性がゼロではないのです。
それゆえ、このセルフホスティング（正確には「自己書き換え」と言うべきかもしれませんが）は、「メモリ領域上に十分に間隔を開けて配置したから、致命的な上書きはきっと起こらないだろう」という、ちょっと雑な目算に基づいています。

本当は start.txt の中において、?STACK の定義の書き換えを行うべきかもしれません。（定義を元に戻すのは、FORTH80.fth からの上書きによって自然に行われる。）

具体的にいうと、?STACK の定義は

    : ?STACK ( --- ) SP@ S0 @ SWAP U< 1 ?ERROR SP@ HERE [ 97 ] LITERAL + U< 7 ?ERROR ;

となっており、これがコンパイルされてメモリ上では

    SP@ S0 @ SWAP U< 1 ?ERROR SP@ HERE LIT 97 + U< 7 ?ERROR ;S
    
の形で（ただし数は 16 進数表示）、?STACK の PFA (parameter field address) 以降の位置に格納されています。
個々のワードはその CFA (compilation field address) で以て代表されます。
つまり、メモリ上では CFA というアドレス値で格納されます。
一方、数はそのまま（16 ビット長の）数として格納されます。

この中の HERE をワード 0 （0 を返す定数としてのワード「0」）に、97 を 6000 （こちらは 16 進数の普通の数）に上書きしておく、というのがここで述べる措置です。

これは ワード「'」や「FIND」及び「!」を使えばできます。
書き換えるべきアドレスは、?STACK の PFA に 16 と 20 をそれぞれ加えたものになります（SP@ -> PFA, S0 -> PFA+2, SWAP -> PFA+4, ..., HERE -> PFA+16, ..., 97 -> PFA+20）。

HERE はシステムが次に書き込むべき位置を示す変数であり、この位置はディクショナリの末尾の次のアドレスでもあります。

97 は、HERE が示す位置に続く、システムがメモ帳代わりに一時的に書着込んで使ってよい領域のバイト数です。
つまり、辞書の末尾に隣接する 0x97 バイト分のメモリ領域は、システムが色々な目的でメモ帳のように使う。

よって、スタックが HERE + 0x79 の位置を超えて伸びてきたとき（スタックは全メモリ領域の後方付近から始まって、アドレスがより小さくなる方に伸びていく）、?STACK は 「Stack full」のエラーを出します、本来は。

これを前記のように書き換えることによって、スタックが 0x6000 の位置を超えて伸びてくることはなくなります。
すなわち、0x6000 よりも若いアドレスがスタックによって破壊されるおそれはなくなります。

ここで述べているセルフホスティングは、システムの実質的本体（本体と核は違う。FORTH の核となる部分はむしろ VSM の範疇）であるディクショナリをアドレス 0x3000 以降に新しく作り直し、次に、その新しいディクショナリを有効にした状態で稼働するシステムによって、古いディクショナリを書き換えることで、機能します。

ここで説明している ?STACK の書き換えは、0x3000 から 0x6000 の間に一時的な新しいディクショナリを作る、ということを前提にしています。

そのようにしておけば、0x3000 から 0x6000 の間にあるディクショナリが最後の書き換え作業時に破壊されるおそれはありません。

しかしながら、私としては、start.txt も仕方なく用意したのだし、その内容は可能な限り最小限にしておきたい。

そもそも、このセルフホスティングにおいて、スタックがそんなに伸びることはないのです。

それゆえ、この措置はあえて取り入れてありません。
