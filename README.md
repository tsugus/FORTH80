...
Windows の場合は、「./VSM -u」と、起動オプション -u をつけて FORTH80 を起動し、start.txt の内容を丸ごとコピペして実行してください。BYE で FORTH80 を終了するときに FORTH80.bin を上書きするか聞かれる（これが -u の意味）ので、「Y」または「y」を入力してください。
...
そちらのエディタも DRIVE0.img のスクリーン 28 以降に収録してあります。
ただし、スクリーン 28 をエラーなくロードするには、前もってスクリーン 17 をロードしておく必要があります。
