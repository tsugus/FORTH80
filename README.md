...
- [FORTH80　 Ver. 0.7.0](#forth80-ver-070)
    - [Abstract](#abstract)
  - [これは何？](#これは何)
  - [使い方](#使い方)
  - [セルフホスティング](#セルフホスティング)
...
FORTH-79 Standard に準拠<sup>*</sup>しています。

（<sup>*</sup> ただし、「block」あたり 1024 バイトを含んでいなければならないという、個人的に不合理に思える規定については除きます。
しかし、FORTH80.bin のソースコードの中でスクリーンあたりのブロック数を表している定数の値 2 を 1 に変更し、なおかつ、バーチャル・スタック・マシン (VSM) のソースコードの中でセクタあたりのバイト数を表しているマクロ定数の値 512 を 1024 に変えれば、形式的に適合させることができます。
〔尤も、個人的にはあまり意義を感じません〕
前者は、FORTH80.asm においてはマクロ定数 BSC0 で、FORTH80.fth においては定数 B/SCR です。
後者は、VSM_win.c および VSM_mac.c のマクロ定数 SECTOR_SIZE です。）
...
けれども、この形式のセルフホスティングは実用性があまりありません。
FORTH のインタプリタの特性を利用して、なるべく手抜きでそれをしようとすることだからです。
つまり、FORTH から仮想機械語へのコンパイラを FORTH で書いたわけではなく、インタプリタに読み込まれる側のプログラムに工夫を施しただけなので、インタプリタを駆動している旧システムと、それに読み込まれるプログラムが表している新システムとの間で基本仕様が同一でないと、世代交代ができずに途中で止まってしまいます。
具体的には、システムの重要パラメータなどはいじれない、ということです。
ところが、そのようなことをしたくなるのはよくあることなので、残念もっばら、このセルフホスティングだけでシステムの改良を行なっていくのは若干無理があり、今までの 8086 アセンブラをうまく利用した方式も残さざるを得ません。

ところがそちらは基本的な機能さえ実現していればよく、基礎に関する仕様変更がない限り、最新である必要がない。
すなわち、そちらのバージョンだけ遅れたものになってしまい、一見ややこしいことになってしまいますが、ご了承ください。
...
つまり、まずエディタを起動して、それに FORTH80.fth の内容を一括してコピペすれば、自動でドライブ 1 の特定の領域（「\<n\> LIST」でスクリーン \<n\> が指定される）に一連の内容が書き込まれます。
最後の改行がコピペされないことがあるので、念のため、改行を押してください。
