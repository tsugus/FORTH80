...
7. 「./VSM -u < start.txt&crarr;」で FORTH80 を起動してセルフホスティングを実行。自動で FORTH80.bin を上書き。（start.txt はリポジトリの src フォルダにあります。したがって、リポジトリをそのままコピーしているならば、「./VSM -u < src/start.txt&crarr;」と入力することになります。）
